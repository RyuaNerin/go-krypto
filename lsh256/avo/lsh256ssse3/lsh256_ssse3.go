...
func lsh256_ssse3_update(ctx *LSH256SSSE3_Context, data Mem, databytelen Register) {
...
func LSH256UpdateSSSE3() {
	TEXT("lsh256UpdateSSSE3", NOSPLIT, "func(ctx *lsh256ContextAsmData, data []byte)")

	ctx := getCtx()
	data := Mem{Base: Load(Param("data").Base(), GP64())}
	databytelen := Load(Param("data").Len(), GP64())

	lsh256_ssse3_update(ctx, data, databytelen)
	Store(ctx.remain_databytelen, Dereference(Param("ctx")).Field("remain_databytelen"))
