...
func initContext(ctx *lsh256Context, size int) {
	ctx.outlenbytes = size
	ctx.Reset()
}

func (b *lsh256Context) Size() int {
	return b.outlenbytes
}

func (b *lsh256Context) BlockSize() int {
	return BlockSize
}

func (b *lsh256Context) Reset() {
...
func (b *lsh256Context) Write(p []byte) (n int, err error) {
	if p == nil || len(p) == 0 {
		return
	}
	plen := len(p)
...
func (b *lsh256Context) Sum(p []byte) []byte {
	b0 := *b
	hash := b0.checkSum()
	return append(p, hash[:b.Size()]...)
}

func (b *lsh256Context) checkSum() [Size]byte {
	b.block[b.boff] = 0x80
...
func (b *lsh256Context) compress(data []byte) {
	b.msgExpansion(data)
...
func (b *lsh256Context) msgExpansion(in []byte) {
	for i := 0; i < 32; i++ {
...
func (b *lsh256Context) step(stepidx, alpha, beta int) {
	var vl, vr uint32
