...
func initContext(ctx *lsh512Context, size int) {
	ctx.outlenbytes = size
	ctx.Reset()
}

func (b *lsh512Context) Size() int {
	return b.outlenbytes
}

func (b *lsh512Context) BlockSize() int {
	return BlockSize
}

func (b *lsh512Context) Reset() {
...
func (b *lsh512Context) Write(p []byte) (n int, err error) {
	if p == nil || len(p) == 0 {
		return
	}
	plen := len(p)
...
func (b *lsh512Context) Sum(p []byte) []byte {
	b0 := *b
	hash := b0.checkSum()
	return append(p, hash[:b.Size()]...)
}

func (b *lsh512Context) checkSum() [Size]byte {
	b.block[b.boff] = 0x80
...
func (b *lsh512Context) compress(data []byte) {
	b.msgExpansion(data)
...
func (b *lsh512Context) msgExpansion(in []byte) {
	for i := 0; i < 32; i++ {
...
func (b *lsh512Context) step(stepidx, alpha, beta int) {
	var vl, vr uint64
