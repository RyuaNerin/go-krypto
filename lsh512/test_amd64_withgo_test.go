...
const (
	testBlocks = 8 * 16 * 1024
)

func Test_SSE2_CONTINUS(t *testing.T)  { testAsmContinus(t, Size, simdSetSSE2, true) }
func Test_SSSE3_CONTINUS(t *testing.T) { testAsmContinus(t, Size, simdSetSSSE3, hasSSSE3) }
func Test_AVX2_CONTINUS(t *testing.T)  { testAsmContinus(t, Size, simdSetAVX2, hasAVX2) }
...
func testAsmContinus(t *testing.T, size int, simd simdSet, do bool) {
	...
	testSize(
		...
		func(t *testing.T, size int) {
			...
			var hGo lsh512ContextGo
			initContextGo(&hGo, size)

			var hAsm lsh512ContextAsm
